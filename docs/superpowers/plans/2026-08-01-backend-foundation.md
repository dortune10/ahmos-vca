...
**Note: this SQL reflects a fix made during execution, not the plan's original draft.** The
first version of this migration defined `auth_app_user()` in the `public` schema with no
`security definer`. Executing it against the real database surfaced two real bugs the
mocked unit tests couldn't catch: (1) `app_user`'s own RLS policy calls this helper, and the
helper selects from `app_user`, so evaluating the policy recursed into itself infinitely
(Postgres error `54001 stack depth limit exceeded`); (2) `get_advisors` flagged that once
`security definer` was added to fix that, the function became callable directly over the
REST API (`anon_security_definer_function_executable` / `authenticated_...`), which
`security definer` functions should never be. The version below is the corrected one that
actually shipped — every plan written after this one (Plans 2, 3, 4, 8) must call
`private.auth_app_user()`, not the unqualified name, since the function no longer exists in
`public` at all.
...
-- exceeded" while testing this task). SECURITY DEFINER makes the internal SELECT run as the
